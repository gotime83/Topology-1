...
Cloudwei.GenericSprite.prototype = {
    ...
    _defaultTooltip: function(){
    	var defaultTooltip = new Kinetic.Text({
	    	id: "DefaultTooltip",
	        text: '',
	        stroke: '#555',
	        strokeWidth: 1,
    ...
    addAnchor: function(config){
    	...
    	anchor.on("mouseover",function(evt){
    		...
    		tooltip.setText("Port : "+this.getId());
    		node.updateTooltipPosition(tooltip,evt);
    ...
    createTooltip: function(evt){
    	...
    	for(var i=0;i<fieldsArray.length;i++){
    		text += (fieldsArray[i] + ' : ' + this.data[fieldsArray[i]]);
    		text += '\n';
    	}
...
Kinetic.Global.extend(Cloudwei.GenericSprite, Kinetic.Group);
